...
- facebook агентство (Агенство): это компания холдер которая предоставляет нам бизнес аккаунты. Для нас это точка входа в Facebook, через них мы пополняем балансы, выводим балансы, перемещаем балансы между аккаунтами
...
- в процессе использования БМ можем измениться привязанный к нему баер. После смены баера затраты аккаунта старые должны остаться в отчете старого баера, а новые затраты должны попасть в затраты нового баера.
...
- как будет производится управление пользователями? Откуда получать список баеров? Как назначать баеру пароль для авторизации в кабинете?
...
| ---------------- | --------- | ------- | ------------------ | -------------------- | ------- | --------------- | ------------- | ----------------- |
| 2026.01.31 12:12 | 892434593 | xxx     | `xx:xx@ff.com:233` | <https://fb.com/apy> | ...     | 200             | ...           |                   |
| 2026.01.31 12:11 | 892434593 | xxx     | `xx:xx@ff.com:233` | <https://fb.com/apy> | ...     | 504             | ...           | proxy timout      |
...
Результат работы workflows мы можем складывать в BQ или PG. Так же в нем удобно реализовать отправку отчетов или алертов (на этапе mvp это out of scope).

Для построения отчетов можем использовать MetaBase.

Все данные внутри таблиц иммутабельные — то есть данные только append only.

Для построения веб интерфейса мы возьмем sveltekit + betterauth для реализации авторизации.

## mocks

Рисовать моки [тут](https://excalidraw.com/#json=Xvab-P9SEGiaXOCDvm2KI,49buGIinfuHqz8SAQLlgmQ)
...
- раздел отчеты
  - bm statuses (все пользователи, фильтр по себе для баеров)
  - spend (все пользователи, фильтр по себе для баеров)
  - inquires status (админы, финансисты)
  - sync log (админы)
- управление БМ (админы)
  - список бм с указанием текущего баера
  - форма назначения баера
