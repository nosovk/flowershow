...
автоматизировать учет затрат в рекламных кабинетах Facebook. Так же реализовать отчетность по затратам в разрезе медабаеров.
...
- баер - пользователь БМ. Мы выдаем БМ конкрентым баерам для настройки и запуска рекламы.
...
- сколько у меня есть фейсбук кабинетов, какой у меня остаток баланса на каждом кабинтее
...
- contact_email: текстовое поле с емейлом
- contact_tg: текстовое поле с tg nickname
- notes: текстовое поле
- status: текущий статус агентсва выставляемый в ручную (тест, активно, архив)
...
спенды зарегистированные в системе
...
- user: кто выполил дейстиве, если оно ручное или initial
- amount: сумма спенда
- currency: валюта в которой учитывается спенд

### balances

балансы зарегистированные в системе
...
- user: кто выполил дейстиве, если оно ручное
...
- в процессе использования БМ можем измениться привязанный к нему баер. После смены баера затрыты аккаунта старые должы остаться в отчета старого баера, а новые затраты должны попасть в затраты нового баера.
...
### spend (с филтром за период - last 7 days)
...
## Technical decisions
поскольу апи fb может лежать, аккаунтов может быть много и т.п. - реализацию сбора данных предлагается реализовать в виде workflows на temporal.io