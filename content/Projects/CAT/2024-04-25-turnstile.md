...
[Посмотреть на тестовом сайте](https://liv-cdn.pages.dev/turnstile) — при загрузке страницы всегда будет показана тестовая аттестация.
...
Дополнение: CF отдельно уточнили что необходимо еще проверять hostname при верификации ответа. Важно это не забыть (в примере этого нет)
### Опционально
