...
Кастомный лендинг (верстаем по макету) для SEO и продвижения (с посадочными страницами для привлечения клиентов и т.п.). Он должен располагаться в корне домена, например vgen.com. Это набор абсолютно статичных HTML-страниц без бэкенда.
...
## Регистрация используя googole auth
Иметь возможность зарегистироваться или залогинится используя google auth кнопку, вместо использования емейла.
...
> Посадочная страница должна содержать информацию о ценах.
> Мое предположение что токены несут неопределенную ценность для людей, поэтому стоимость токена пользователю ничего не скажет. Делать оценку в минутах мне кажется плохой идеей, так как это может привести к проблемам с ценообразованием (разные модели в одном продукте, с разной стоимостью, невозможность опираться на входящие данные используемые для генерации и т.п.).
...
Лендинг должен поддерживать многоязычность. Основной язык - английский. Формат страниц должен быть вида:

- /: главная страница на английском
- /uk: главная страница на украинском
- /terms: страница с правилами использования (только английский)
- /privacy: страница с политикой конфиденциальности (только английский)


SEO-оптимизация для лендинга ограничивается следующими настройками:
- meta-теги
  - title
  - description
- json-ld
...
Я не вижу смысла интегриовать систему переводов в лендинг из пары страниц, поэтому предлагаю просто сделать страницы с разными языками руками.

## Приложение (APP)
Приложение является MPA с поддержкой session-based авторизации. Приложение не требует SEO или SSR, поскольку отсутствует доступ для неавторизованных пользователей. Приложение должно поддерживать многоязычность. Мы хотим редактировать переводы через weblate.

В дизайне приложения мы используем стандартные компоненты — максимально простой, но функциональный дизайн. Мы не создаем custom-вёрстку и не делаем специальных элементов интерфейса.
...
prices = [
  ...
  tokens: 30_000,
  discount: 5, // размер скидки в процентах
  },
   {
  name: "large",
  price: 5000, // 50usd, denomination 100
  tokens: 70_000,
  discount: 10, // показывается сверху панельки с пакетом, если отлична от 0
  }
]
```
...
### Безопасность
Приложение публикуется на Cloudflare. При необходимости активации DDoS-защиты, rate-limiting и т.д. мы используем функционал, встроенный в Cloudflare поверх домена, на котором публикуем приложение. Каких-то дополнительных механизмов защиты мы не предусматриваем.

Время жизни сессии — 7 дней. Сброс сессии происходит при смене страны или смене User-Agent.

### Файловое хранилище и валидация загрузок
Приложение выполняет только клиентскую валидацию файлов (проверка размера и расширения). Мы ожидаем, что нам предоставят список допустимых расширений, и, например, мы ограничим максимальный размер загружаемого файла.

> Важно: Cloudflare на недорогих тарифах имеет жёсткий лимит в 100 МБ на максимальный размер payload.

Мы не сохраняем файлы на уровне приложения — сразу передаём их дальше в API продукта. Никакой обработки файлов не происходит.

**Предполагаемые ограничения:**
- Максимальный размер файла: 100 МБ
- Поддерживаемые форматы видео: MP4, MOV, AVI
- Поддерживаемые форматы аудио: MP3, WAV, M4A
- Максимум одновременных заданий на пользователя: 3

## Email-уведомления
Фактически мы используем электронную почту только для верификации email-адреса. Для отправки писем предлагаем использовать Mailgun (необходимо будет предоставить нам аккаунт или API-ключи от него).
...
## User Engagement
Для работы сcustomer journey мы предлагаем инетгрировать [customer.io](https://customer.io/features/in-app-messages). Его можно использовать для отправки рассылок пользователям. Для интеграции сustomer.io мы планируем использовать bigquery как источник данных для Customer. При этом на сайте нам необходимо будет установить их JS код интеграции.


## Environments и развёртывание

### Окружения
Мы поднимем два окружения — staging и production. Для удобства мы разносим их на разные домены:

- **Staging:** находится в доменной зоне pages.dev и в отдельном Cloudflare Workers проекте
- **Production:** привязывается к production-домену

При этом база D1, KV и environment-переменные у окружений полностью независимы.


## Риски
Могут возникнуть проблемы с передачей файлов в API. Workers имеют много ограничений, связанных со временем исполнения, что может создать нам проблемы.

Могут возникнуть проблемы с API (мы попросили внести некоторые изменения в API, которые мы ещё ожидаем).

Файлы удаляются спустя некоторое время после генерации, это может привести к конфликтным ситуациям с пользователями (т.к. задания, как мы понимаем, будут пропадать из списка). Возможно, необходимо сделать где-то большой дисклеймер о том, что файлы нужно скачивать себе.

Ссылка на итоговый файл ведёт на домен API, что может привести к его раскрытию конечными пользователями и прямой атаке на него (рекомендуем реализовать сохранение сгенерированных файлов в Cloudflare R2).
