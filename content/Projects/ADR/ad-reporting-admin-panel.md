...
Как механизм визуализации мы используем metabase. Это означени что мы можем быстро конфигурировать views в bigquery и выводить их как embedded блок в виде iframe внутри продукта. 
### ACL
Проблема состоит только в том, что metabase read only база данных для metabase. Поэтому мы вынужденны завести несоклько новых таблиц, для управления доступами.
#### Таблица users
...
На этом экране должна быть опция добавить пользователя, или отредактировать уже сущесвтующих пользователей. Экран доступен только для ролей admin либо teamlead (с фильтром по parent). Оптимально сделать хранение этих параметров через [EAV](./eav-explanation), по аналогии с тем, как это сделанно было в SOT. В этом случае мы будем иметь так же историю изменения прав доступа.
### Экран аккаунтов
...
### Экран Цели
Экран необходим для установки целей для аккаунтов. Цели так же предлагается в виде [EAV](./eav-explanation) хранить. Таким образом если когда-то у аккаунтов появятся какие-то дополнительные поля - мы легко сможем расширить представление.
![goals screen](./attachments/adr-goals.png)
### Экран профиля
