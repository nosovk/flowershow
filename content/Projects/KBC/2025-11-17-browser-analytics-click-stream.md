...
5. Сервер обогащает данные (к примеру по userid берет данные профиля на момент записи) и складывает в плоскую таблицу в DB (у нас это BigQuery)

## JS интеграция
Важные для нас поля, которые должны быть в каждом пакете:
- type: "click"|"load"|"scroll", etc.
- domain: "https://kingbillycasino.com"
- browserID: "uid", уникальный идентификатор браузера (fp или timestamp)
- sessionID: "uid", уникальный идентификатор конркетного окна (sessionStorage scoped)
